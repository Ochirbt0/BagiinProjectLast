    ...
    const syncStars = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setDisplayStars(Number(payload?.stars ?? 0));
          localStorage.setItem("userStars", String(payload?.stars ?? 0));
        }
      } catch {}
    };
    syncStars();
    ...
    >
      <div className="container mx-auto flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {!isSignedIn ? (
            <SignInButton mode="modal">
              <button className="flex items-center gap-2 px-5 py-2 md:px-7 md:py-2.5 bg-[#5D3191] text-white font-black text-[10px] md:text-[12px] uppercase rounded-[16px] shadow-lg">
