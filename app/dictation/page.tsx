...
export default function DictationPage() {
  ...
  const [anagramShuffled, setAnagramShuffled] = useState<Record<string, string>>({});
    ...
    const loadSentence = async () => {
      ...
      try {
        ...
        const sentences = Array.isArray(data?.sentences)
          ? data.sentences.map((item) => String(item || "").trim()).filter(Boolean)
          : [];
    ...
    <div className="min-h-screen bg-[#FDFCFE] flex flex-col items-center font-sans overflow-x-hidden">
      ...
      <main className="w-full max-w-5xl flex-1 flex flex-col relative z-10 px-4 md:px-6 pt-24 md:pt-32 pb-10">
          ...
          <div className="px-6 md:px-12 py-8 flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <h1 className="text-xl md:text-3xl font-black text-[#5D3191] tracking-tight">{topicTitle}</h1>
              </div>
              ...
              <button
                ...
                className={`hidden md:flex items-center gap-2 px-6 py-3 rounded-xl font-black text-xs transition-all shadow-md disabled:opacity-60 ${
                  isPlaying ? "bg-amber-400 text-white animate-pulse" : "bg-[#5D3191] text-white"
                }`}
              >
                <Volume2 size={18} /> {isPlaying ? "Тоглож байна" : "Сонсох (3x)"}
              </button>
            </div>

            <div className="flex items-center gap-3 md:gap-4">
              <p className="text-slate-400 font-bold text-[15px]">{statusText}</p>
              <button
                onClick={handlePlay}
                disabled={!sourceSentences.length || isPlaying || isGenerating}
              ...
              >
                <Volume2 size={12} /> {isPlaying ? "Тоглож байна" : "СОНСОХ (3x)"}
              </button>
            </div>
          </div>
          ...
          <div className="flex-1 px-4 md:px-12">
            <div className="relative h-full min-h-[320px] md:min-h-[350px] bg-slate-50/30 rounded-[28px] border border-slate-100 shadow-inner overflow-hidden">
              <div
                ...
                style={{
                  backgroundImage: "linear-gradient(#5D3191 1px, transparent 1px)",
        ...
        <div className="mt-6 bg-amber-50/50 border border-amber-100 rounded-[25px] p-6 flex gap-4 items-start">
          ...
          <div>
            <h4 className="text-amber-700 font-black text-[10px] uppercase tracking-widest">Үр дүн</h4>
            {error ? (
              ...
              <div className="text-amber-800/80 text-sm font-bold mt-1 leading-tight">
                {typeof result.score === "number" && <p>Оноо: {result.score}</p>}
                {typeof result.totalScore === "number" && (
                  <p>Нийт оноо: {result.totalScore}</p>
                )}
                {typeof result.errors === "number" && <p>Алдаа: {result.errors}</p>}
                {isTopicPassed ? (
                  <p className="text-green-600">Статус: Энэ сэдвийг давлаа.</p>
                ) : (
                  <p className="text-amber-700">Статус: Анаграм даалгавар дутуу.</p>
                )}
      ...
      <AnimatePresence>
        {showAnagram && (
          ...
          >
            ...
            >
              <h3 className="text-[#5D3191] text-xl font-black mb-2">Анаграм засвар</h3>
              <p className="text-slate-500 text-sm font-bold mb-4">
                Алдаатай үг бүрийг зөв тааруулж бичсэний дараа энэ сэдэв давсанд тооцогдоно.
              </p>

              <div className="space-y-3 max-h-[45vh] overflow-auto pr-1">
                {anagramWords.map((word) => {
                  return (
                    <div key={word} className="p-3 rounded-2xl border border-slate-100 bg-slate-50/60">
                      <p className="text-xs text-slate-500 font-black mb-1">Анаграм</p>
              ...
              {anagramError && (
                <p className="text-red-500 text-sm font-bold mt-3">{anagramError}</p>
              )}
